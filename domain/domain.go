...
func (do *Domain) loadInfoSchema(handle *infoschema.Handle, usedSchemaVersion int64, startTS uint64) error {
	...
	if usedSchemaVersion != 0 && usedSchemaVersion == latestSchemaVersion {
		log.Errorf("[ddl] schema version is still %d, no need reload, s:%v", usedSchemaVersion, do.store.UUID())
		return nil
	}
	...
	if ok {
		log.Errorf("[ddl] diff load InfoSchema from version %d to %d, s:%v", usedSchemaVersion, latestSchemaVersion, do.store.UUID())
		return nil
	}
	...
	log.Errorf("[ddl] full load InfoSchema from version %d to %d, s:%v", usedSchemaVersion, latestSchemaVersion, do.store.UUID())
...
func (do *Domain) reload() error {
	...
	for {
		...
		log.Errorf("[ddl:%s] load schema, ver:%v, time:%v", do.store.UUID(), ver.Ver, time.Now())
		if err == nil {
...
func (do *Domain) MustReload() error {
	log.Errorf("[ddl:%s] must reload, time:%v", do.store.UUID(), time.Now())
...
func (do *Domain) checkValidityLoop(lease time.Duration) {
	...
	for {
		...
		case <-timer.C:
			...
			log.Errorf("[ddl:%s] loop, sub:%v, lease:%v, succ:%v, waitTime:%v, now:%v",
